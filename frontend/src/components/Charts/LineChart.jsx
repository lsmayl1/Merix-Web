...
export const LineChart = ({ data, valueKey = "revenue" }) => {
  ...
  let labels = data ? data.map((item) => item.date) : [];
  let dataPoints = data ? data.map((item) => item[valueKey]) : [];

  if (dataPoints.length === 1) {
    labels = ["", labels[0]]; // Y ekseninden başlasın
  ...
  const options = {
    ...
    scales: {
      x: {
        ...
        offset: false, // X ekseninin daha düzgün hizalanması için ekledik
        ticks: {
