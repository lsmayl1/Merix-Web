      ...
      <ul className="grid grid-cols-3 gap-2  pr-2 overflow-auto min-h-0 py-2 ">
        {data?.map((item, index) => (
          ...
          >
            ...
            <h1 className="text-flg font-medium w-9/12">{item.name}</h1>
            <div className="flex justify-between w-full">
              <h1 className="text-lg text-nowrap ">{item.sellPrice} ₼</h1>

              <QtyInput
                barcode={item.barcode}
                qty={
                  existProduct?.find((x) => x.barcode == item.barcode)?.quantity
                }
                handleQty={handleChangeQty}
                allign={"justify-end gap-2"}
                className={
                  existProduct?.find((x) => x.barcode == item.barcode)
                    ? "bg-blue-600 text-white "
                    : "bg-white"
            ...
            {openContext === item.product_id && (
              <button
                onClick={(e) => handleDeleteShortcut(item.product_id, e)}
