...
export const SupplierDetails = () => {
      ...
      cell: ({ row }) => (
        <div className="flex justify-center  gap-4">
          {row.original.reference_no ? (
            `${row.original.reference_no + ` Qaytarilib`}`
          ) : (
            <button
              className="cursor-pointer"
              onClick={() => handleReturnTransaction(row.original.id)}
            >
  ...
  const [returnTransaction] = useReturnSupllierTransactionMutation();
  ...
  const handleReturnTransaction = async (id) => {
    try {
      returnTransaction(id).unwrap();
      await refetch();
    } catch (error) {
      toast.error(error);
    }
  ...
  return (
    <div className="p-4 flex flex-col gap-4 w-full h-full relative">
      ...
      <div className="flex flex-col bg-white w-full h-full  rounded-lg   min-h-0 ">
        <ToastContainer />
        {showInvoice && (
          <InvoiceView
            handleClose={() => {
