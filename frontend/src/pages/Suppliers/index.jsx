      ...
      <div className="flex items-center gap-2 w-full">
        <KPI
          data={[
            ...
            {
              label: "Təchizatçı Sayı",
              value: metric?.supplierCount,
            },
          ]}
        />
      </div>
      <div className="bg-white w-full h-full rounded-lg overflow-auto p-4 gap-4 flex flex-col ">
        {showModal && (
          <Modal
            onSubmit={onSubmit}
            isOpen={showModal}
        ...
        <div className="flex items-center justify-between ">
          ...
          <MainButton title={t("add")} handleClick={() => setShowModal(true)} />
        </div>
        <div className="grid grid-cols-1 gap-2 pr-4 overflow-auto">
          {data?.map((supplier) => (
            <NavLink
              key={supplier.id}
              to={`/suppliers/${supplier.id}`}
              className="bg-white rounded-lg hover:shadow-lg transition-shadow duration-300 p-4 flex items-center justify-between border border-gray-200"
            >
