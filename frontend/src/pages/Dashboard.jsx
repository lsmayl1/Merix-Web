...
import { LastTransactions } from "../components/Supplier/LastTransactions.jsx";

export const Dashboard = () => {
  const { t } = useTranslation();
  const [timeframeVariable] = useState([
    { label: "D", value: "daily" },
    { label: "H", value: "weekly" },
    { label: "A", value: "monthly" },
  ]);
  ...
  return (
    <div className="w-full h-screen  flex flex-col pr-2 max-md:gap-1 gap-2 min-h-0  ">
      ...
      <div className="bg-white w-full min-h-[400px] h-full rounded-lg p-2 ">
        <h1 className="text-md text-mainText font-medium">Son ödənişlər</h1>
        <LastTransactions />
      </div>
    </div>
  );
