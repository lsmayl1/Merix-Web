  ...
  try {
    ...
    const products = await Products.findAll({
      ...
      attributes: [
        ...fields,
        [
          sequelize.fn("SUM", sequelize.col("stockBatches.remainingQuantity")),
          "totalStock",
        ],
      ],
      include: [
...
const CreateProduct = async (product) => {
  try {
    ...
    const newProduct = await Products.create({ ...product, isDeleted });
  ...
  try {
    ...
    if (unit === "piece") {
      do {
        ...
        // Ensure barcode doesn't start with "22"
        while (newBarcodeBase.startsWith("22")) {
          newBarcodeBase = Math.floor(
            100000000000 + Math.random() * 900000000000,
          ).toString();
        }
