...
const { UpdateStockValue } = require("../product/product.service");
...
router.get("/all", async (req, res) => {
  try {
    const transactions = await getAllSupplierTransactions();
    if (!transactions || transactions.length === 0) {
      return res
        .status(404)
        .json({ message: "No transactions found", transactions: [] });
...
router.post("/return-transaction/:id", async (req, res, next) => {
  try {
    const transaction = await ReturnTransaction(req.body.id);
...
router.post("/v2/:transaction_id", async (req, res, next) => {
  try {
